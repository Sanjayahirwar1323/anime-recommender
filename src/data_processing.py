...
class DataProcessor:
    ...
    def save_artifacts(self): ## arrays can't be save in csv or something so we have to store them in pkl format therefore below we are using joblib
        try:
            artifacts = { 
                "user2user_encoded" : self.user2user_encoded,## name of the file is user2user_encoded.pkl and it will save user2user encoded data in it
                "user2user_decoded" : self.user2user_decoded, ## this will save in processed directory path is given in path_config.py
                "anim2anime_encoded" : self.anime2anime_encoded,
                "anim2anime_decoded" : self.anime2anime_decoded,
            }

            for name,data in artifacts.items(): ## loop as it will have  one name and other data 
                joblib.dump(data, os.path.join(self.output_dir,f"{name}.pkl")) ## name of the file will be user2user_encoded.pkl and it will save user2user encoded data in it
                logger.info(f"{name} saved sucesfully in processed directory") ## output_dir is processed directory
